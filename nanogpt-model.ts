...
import { MX, fromI32, sample, tidy, saveSafetensors } from "./mx.ts";
import { loadSafetensors, get } from "./loader.ts";
import { treeFlatten, type Tree } from "./pytree.ts";
...
export async function loadCkpt(ckpt: string): Promise<{ params: Tree; cfg: Cfg }> {
  const cfg: Cfg = await Bun.file(cfgPath(ckpt)).json();
...
export async function saveCkpt(ckpt: string, params: any, cfg: Cfg) {
  ...
  await Bun.write(cfgPath(ckpt), JSON.stringify(cfg, null, 2));
}
