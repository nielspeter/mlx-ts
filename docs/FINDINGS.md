...
(`../scripts/validate-all.sh`: 49/49). It also **trains transformers from scratch** —
...
All fifteen are re-checked together by `../scripts/validate-all.sh` — **49/49 green**
...
(49/49) forced onto the bundled path.
...
token for token, and `../scripts/validate-all.sh` is **49/49 forced onto it**.
...
- **`../scripts/validate-all.sh`** — the full suite: every TS path vs its reference (49/49)
