...
const SHADOWS = new Set(
  "m r ptr stream asBig kptr cstr optFloatU64 optIntU64 Arr Vec Dtype".split(" "));
...
const symbolLines: string[] = [
  // Hand-added: error.h declares this with function-pointer parameters, which
  // the parser below has no CType mapping for. Without it mlx-c keeps its
  // default handler, which prints and calls exit() — see the preamble.
  `  mlx_set_error_handler: { args: ["ptr", "ptr", "ptr"], returns: "void" },`,
];
...
for (const fn of allFns) {
  ...
  wrapperLines.push(
    ...
    `  throwIfError("${fn.name}", Number(r[0]));\n` +
    `  return Number(r[0]);\n}`,
  );
  wrapCount++;
}
...
import { callback, cstring, open, ptr } from "./index.ts";
...
// synchronous call it is passed to, and mlx_array_new_data copies its input —
// verified by mutating the source buffer before any eval and reading the
// original values back. Retaining every shape buffer and C string for the life
// of the process grew the JS heap with each token and each reshape.
const kptr = (a: ArrayBufferView) => ptr(a);
...
function cstr(s: string) { return ptr(new Uint8Array([...new TextEncoder().encode(s), 0])); }

// mlx-c's default error handler prints the message and calls exit(). That turns
// any bad shape — including one derived from an HTTP request — into a dead
// process that no try/catch can intercept. Record it instead, and let the call
// site raise a normal TypeScript Error.
//
...
/**
 * Raise anything mlx-c reported since the last check.
 *
 * Cheap enough for the hot path: one null comparison per operation. \`op\` names
 * the call for the message; \`orphan\` is a handle allocated for the result that
 * would otherwise leak when we throw past it.
 */
export function throwIfError(op: string, orphan?: number): void {
  if (LAST_ERROR === null) return;
  const msg = LAST_ERROR;
  LAST_ERROR = null;
  if (orphan) m.mlx_array_free(orphan);
  throw new Error(\`\${op}: \${msg}\`);
}

/** Whether mlx-c has reported an error that no one has consumed yet. */
export const pendingError = (): string | null => LAST_ERROR;
...
export function show(a: Arr): string {
  // mlx_string_data is bound as a pointer return, so the cast this used to do
  // handed back a number typed as a string. Decode it, and free the temporary.
  const s = new BigUint64Array(1); s[0] = asBig(m.mlx_string_new());
  try {
    m.mlx_array_tostring(ptr(s), a);
    throwIfError("mlx_array_tostring");
    return cstring(Number(m.mlx_string_data(Number(s[0]))));
  } finally {
    m.mlx_string_free(Number(s[0]));
  }
}
`;
