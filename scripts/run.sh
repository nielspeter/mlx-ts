...
#
#   bash scripts/run.sh                  # ~15-20 min on an M-series Mac (defaults below)
#   MAX_BYTES=20000000 BASE_ITERS=800 bash scripts/run.sh   # quicker, rougher
set -e
cd "$(dirname "$0")"
...
STORIES=$CORPUS LR=${SFT_LR:-3e-4} ITERS=${SFT_ITERS:-1500} SFT_N=${SFT_N:-1200} bun training/chat-sft.ts

echo "=== [5/5] chat ==="
TEMP=0.7 bun training/chat-ckpt.ts "Tell me a story about a cat."
TEMP=0.7 bun training/chat-ckpt.ts "Tell me a story about a robot."
echo
echo "Done. CLI:  TEMP=0.7 bun training/chat-ckpt.ts \"<question>\"    Web UI:  bun examples/chat-web.ts  -> http://localhost:8080"
