...
— so it runs in native Rust. The output tokenizer.json is consumed by our pure-TS
inference (`tokenizer.ts`), which is validated token-exact.
  VOCAB=2048 CORPUS=input.txt python3 tok-train.py
"""
import os, json
from tokenizers import Tokenizer, models, pre_tokenizers, decoders, trainers

VOCAB = int(os.environ.get("VOCAB", 2048))
CORPUS = os.environ.get("CORPUS", "input.txt")
...
tok.save("tokenizer-trained.json")
...
json.dump(out, open("tok-trained-fixtures.json", "w"))
print(f"trained byte-level BPE: vocab={tok.get_vocab_size()} on {CORPUS} -> tokenizer-trained.json")
